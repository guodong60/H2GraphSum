...
import sys
# sys.path.append('../../')
from ..nl_parser.en_parser import punc_str
from .code_tokenizer import tokenize_code_str
from tree_sitter import Language, Parser,Node
import os,re
from collections import deque
from ._DFG_CFG import *
...
class SitParser(object):
    ...
    def CFG_edges(self):
        ...
        CFG,CFG_edges=[],[[],[]]
        if self.language=='python':
            CFG, _ = CFG_python(root_node=self._tree.root_node, point2code=ast_node_point2node_id_and_code)
        if self.language=='java':
            CFG, _ = CFG_java(root_node=self._tree.root_node, point2code=ast_node_point2node_id_and_code)
        #DFG is the structure like this ('b', 11, 'comesFrom', [], []), ('x', 25, 'comesFrom', ['x'], [43, 49]), ('x', 26, 'computedFrom', ['0'], [28])
        if not self.ast_intact: #如果是精简树，建立全树节点id到简树节点id的映射，去掉冗余节点
            intact_ast_node_id2ast_concise_node_id=dict(zip([node_id for node_id,tags in
                                                             zip(list(range(len(self._intact_ast_nodes))),
                                                            ...
                                                            list(range(len(self._concise_ast_nodes)))))
        for edge_obj in CFG:
            edge_obj_4=edge_obj[4] if isinstance(edge_obj[4],list) else [edge_obj[4]]  #如果是单个节点则转为列表
            for start_node_id in edge_obj_4:
                if self.ast_intact:
                    CFG_edges[0].append(edge_obj[1])
                    CFG_edges[1].append(start_node_id)
                elif start_node_id in intact_ast_node_id2ast_concise_node_id.keys() and \
                    edge_obj[1] in intact_ast_node_id2ast_concise_node_id.keys():   #如果是简树
                    CFG_edges[0].append(intact_ast_node_id2ast_concise_node_id[edge_obj[1]])
                    CFG_edges[1].append(intact_ast_node_id2ast_concise_node_id[start_node_id])

        return np.array(CFG_edges)
