...
class SinusoidalPositionalEncoding(nn.Module):
    ...
    优点:
    1. 不需要训练，节省参数
    2. 泛化能力强，可以处理训练时未见过的序列长度
    3. 具有周期性，相似位置编码相似
    """
    def __init__(self, emb_dims, max_len=5000, pad_idx=0):
        """
        Args:
            emb_dims: embedding维度
            max_len: 支持的最大序列长度
            pad_idx: padding索引
        """
        super().__init__()
        ...
        # 创建位置编码矩阵 [max_len+1, emb_dims]
        # +1是为了包含padding位置(索引0)
        position_code = np.zeros((max_len + 1, emb_dims))
        
        # 位置索引 [1, 2, 3, ..., max_len]
        # 注意: 位置0保留给padding，从1开始
        position = np.arange(1, max_len + 1).reshape(-1, 1)  # [max_len, 1]
        
        # 频率因子
        # div_term = 10000^(2i/d_model), i=0,1,2,...,d_model/2
        div_term = np.exp(
            np.arange(0, emb_dims, 2) * -(np.log(10000.0) / emb_dims)
        )  # [emb_dims/2]
        
        # 偶数维度使用sin
        position_code[1:, 0::2] = np.sin(position * div_term)
        # 奇数维度使用cos
        position_code[1:, 1::2] = np.cos(position * div_term)
        
        # 位置0 (padding) 全为0
        position_code[0, :] = 0.0
        
        # 注册为buffer（不参与训练，但会保存在模型中）
        self.register_buffer('position_code', torch.FloatTensor(position_code))
        
    def forward(self, positions):
        """
        Args:
            positions: [batch_size, seq_len] 或 [num_nodes] 位置索引
        Returns:
            pos_emb: 对应的位置编码
        """
        return self.position_code[positions]
