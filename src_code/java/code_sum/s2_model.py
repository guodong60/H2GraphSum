...
class Datasetx(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data['node','parentparent_child_hyperedges','node'].edge_index=torch.tensor(self.code_graphs[index]['parent_child_hyperedges']).long()
        ...
        for _ in range(graph_gnn_layers):
            if graph_GNN==SAGEConv:
                ...
                gnn=HeteroConv({
                    ...
                    ('node', 'parentparent_child_hyperedges', 'node'): HypergraphConv((emb_dims,emb_dims), emb_dims, aggr=graph_gnn_aggr,root_weight=False),
