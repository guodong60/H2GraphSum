    ...
    for item in pbar:
       ...
       #数据流超边
        ...
        for parent, children in parent_to_children.items():
            ...
            hedge_id += 1
        # 3. 拼成 numpy array
        parent_child_hyperedges = np.array([nodes_list, hyperedge_id_list])
        
        #行级超边
        line_token_poses = parser_dict['code_token_poses']
        line_token_edges = parser_dict['code_token_edges']

        # 按行号收集 code_token_poses 的索引
        line_to_indices = defaultdict(list)
        for idx, (line, _) in enumerate(line_token_poses):
            line_to_indices[line].append(idx)

        nodes = line_token_edges[0]  # 第一行，节点索引数组
        extra_node = line_token_edges[1, -1]  # 第二行最后一个节点，待加入最后一条超边
        nodes = np.append(nodes, extra_node) # 将 extra_node 加入到 nodes 中
        node_list = []
        hyperedge_list = []
